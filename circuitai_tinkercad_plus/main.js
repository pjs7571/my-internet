...
function initDragAndDrop(){
  ...
  ws.addEventListener('mouseup', e=>{ if(isWiring && !e.target.classList.contains('pin')) addRoutePoint(e); });
...
function addComponentToWorkspace(type,x,y){
  ...
  document.addEventListener('mouseup',()=>{ if(drag){drag=null; el.style.zIndex=10; snapComponentToBreadboard(el); updateWires(el); saveAuto(); }});
  updateVisuals(el); renderInspector(); return el;
}
...
function createVisualComponent(type){
  ...
  let w=60,h=40;
  if(base==='breadboard'){
    w=465; h=270; el.style.width=w+'px'; el.style.height=h+'px'; el.innerHTML=`<div class="bb-body"><div class="bb-line b-blue" style="top:18px"></div><div class="bb-line b-red" style="top:40px"></div><div class="bb-line b-red" style="top:226px"></div><div class="bb-line b-blue" style="top:248px"></div><div class="bb-groove"></div></div>`; const body=$('.bb-body',el);
    for(let c=0;c<30;c++){addPin(body,`pwr+${c}`,20+c*14,28,'+'); addPin(body,`pwr-${c}`,20+c*14,52,'-'); addPin(body,`gnd+${c}`,20+c*14,218,'+'); addPin(body,`gnd-${c}`,20+c*14,242,'-'); for(let r=0;r<5;r++){addPin(body,`a${r}-${c}`,20+c*14,86+r*14,String.fromCharCode(97+r)); addPin(body,`f${r}-${c}`,20+c*14,146+r*14,String.fromCharCode(102+r));}}
...
function startWire(pin){if(currentLine) cancelWire(); isWiring=true;startPin=pin;const p=pinPos(pin);currentLine=document.createElementNS('http://www.w3.org/2000/svg','line');currentLine.setAttribute('x1',p.x);currentLine.setAttribute('y1',p.y);currentLine.setAttribute('x2',p.x);currentLine.setAttribute('y2',p.y);currentLine.setAttribute('stroke','#334155');currentLine.setAttribute('stroke-width','4');currentLine.setAttribute('stroke-linecap','round');currentLine.startPin=pin;currentLine.isConnected=false;currentLine.style.pointerEvents='none';pin.lines.push(currentLine);svg.appendChild(currentLine);}
function finishWire(pin){if(isWiring&&startPin!==pin&&currentLine){const p=pinPos(pin);currentLine.setAttribute('x2',p.x);currentLine.setAttribute('y2',p.y);currentLine.endPin=pin;currentLine.isConnected=true;currentLine.style.pointerEvents='stroke';pin.lines.push(currentLine);isWiring=false;startPin=null;currentLine=null;saveAuto(); if(running)runSimulation();}}
...
function snapComponentToBreadboard(el){ if(el.dataset.baseType==='breadboard'||el.dataset.baseType==='route'||['battery','dc','arduino'].includes(el.dataset.baseType))return; const pins=$$('.pin',el); if(!pins.length)return; const target=nearestBreadboardHole(pins[0],18); if(!target)return; const a=pinPos(pins[0]), b=pinPos(target); el.style.left=snap(parseFloat(el.style.left)+(b.x-a.x))+'px'; el.style.top=snap(parseFloat(el.style.top)+(b.y-a.y))+'px'; }
...
function buildGraph(){const nodes=new Map(); const add=(a,b)=>{if(!nodes.has(a))nodes.set(a,new Set());nodes.get(a).add(b);}; $$('line',svg).forEach(l=>{if(l.startPin&&l.endPin){add(l.startPin,l.endPin);add(l.endPin,l.startPin);}}); return nodes;}
...
function runSimulation(){running=true; $('#simStatus').textContent='실행중'; $('#simStatus').className='status running'; $$('#wire-svg line').forEach(l=>l.classList.remove('powered-wire')); $$('.led-graphic').forEach(g=>g.classList.remove('led-on','led-burn')); $$('.motor-dc-graphic,.motor-servo-graphic').forEach(g=>g.classList.remove('motor-on')); $$('.buzzer-graphic').forEach(g=>g.classList.remove('buzz-on'));
 const graph=buildGraph(); let messages=[]; let poweredPins=new Set(); let batteries=$$('.workspace-component').filter(c=>['battery','dc'].includes(c.dataset.baseType));
 batteries.forEach(b=>{const bp=$$('.pin',b), vcc=bp.find(p=>pinName(p)==='vcc'), gnd=bp.find(p=>pinName(p)==='gnd'), V=parseFloat(b.dataset.voltage)||5; if(!vcc||!gnd)return; $$('.workspace-component').forEach(comp=>{const base=comp.dataset.baseType; if(base==='led'){const pins=$$('.pin',comp), a=pins.find(p=>pinName(p)==='a'), k=pins.find(p=>pinName(p)==='k'); if(reachable(graph,vcc,a)&&reachable(graph,k,gnd)){let r=findSeriesResistance(graph,a,vcc)+findSeriesResistance(graph,k,gnd); let current=r>0?(V-2)/r:999; const led=$('.led-graphic',comp); if(r===0||current>.04){led.classList.add('led-burn'); messages.push(`LED 과전류 위험 ${current===999?'∞':(current*1000).toFixed(1)}mA`);} else if(current>.002){led.classList.add('led-on'); markPath(graph,vcc,a); markPath(graph,k,gnd); messages.push(`LED ON / 약 ${(current*1000).toFixed(1)}mA`);} }}
 if(base==='motor'||base==='buzzer'){const p=$$('.pin',comp), p1=p.find(x=>pinName(x)==='p1'), p2=p.find(x=>pinName(x)==='p2'); if(reachable(graph,vcc,p1)&&reachable(graph,p2,gnd)){markPath(graph,vcc,p1); markPath(graph,p2,gnd); const g=$('.graphic',comp); if(base==='motor')g.classList.add('motor-on'); if(base==='buzzer')g.classList.add('buzz-on'); messages.push(`${comp.dataset.type} 동작`);}}
 });});
 applyArduinoOutputs(graph,messages); $('#electricInfo').textContent=messages.length?messages.slice(0,3).join(' · '):'닫힌 회로를 찾지 못했습니다'; saveAuto();}
...
function saveCircuit(){localStorage.setItem('circuitAIProject',JSON.stringify(serializeV2())); alert('저장 완료');}
function saveAuto(){localStorage.setItem('circuitAIProjectAuto',JSON.stringify(serializeV2()));}
function autoLoad(){const d=localStorage.getItem('circuitAIProjectAuto'); if(d)try{deserialize(JSON.parse(d));}catch(e){}}
function loadCircuit(){const d=localStorage.getItem('circuitAIProject')||localStorage.getItem('circuitAIProjectAuto'); if(!d)return alert('저장된 회로가 없습니다.'); deserialize(JSON.parse(d));}
function exportCircuit(){const blob=new Blob([JSON.stringify(serializeV2(),null,2)],{type:'application/json'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='circuitai-project.json'; a.click(); URL.revokeObjectURL(a.href);}
...
function connectPins(a,b,color='#334155'){const p1=pinPos(a),p2=pinPos(b); const line=document.createElementNS('http://www.w3.org/2000/svg','line'); line.setAttribute('x1',p1.x);line.setAttribute('y1',p1.y);line.setAttribute('x2',p2.x);line.setAttribute('y2',p2.y);line.setAttribute('stroke',color);line.setAttribute('stroke-width','3');line.setAttribute('stroke-linecap','round'); line.startPin=a; line.endPin=b; line.isConnected=true; line.style.pointerEvents='stroke'; a.lines.push(line); b.lines.push(line); svg.appendChild(line);}
